...
std::string auth_check_nigga_shit = XorStr("false").c_str();
namespace KeyAuth {
	...
	public:
		...
		void license(std::string key) {
			...
			if (json[("success")])
			{
				auth_check_nigga_shit = XorStr("true").c_str();
				// optional success message
				//load_user_data(json[("info")]);
			}
			else
	...
	private:
		...
		static std::string req(std::string data) {
			...
			curl_easy_setopt(curl, CURLOPT_PINNEDPUBLICKEY, "sha256//WUeCfm+ayuWAtn9nD1HsQ6coO32Iu+/p+vL9OIYvLZs=");
